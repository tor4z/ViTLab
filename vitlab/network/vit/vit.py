...
class ModifiedResNet(nn.Module):
    """
    A ResNet class that is similar to torchvision's but contains
        the following changes:
    - There are now 3 "stem" convolutions as opposed to 1,
        with an average pool instead of a max pool.
    - Performs anti-aliasing strided convolutions, where an
        avgpool is prepended to convolutions with stride > 1
...
class VisualTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        """
        A Vision Transformer with sliding patch and
        ...
        for patch_sz, stride_sz in zip(patch_sizes, patch_strides):
            ...
            num_patches += (
                (input_resolution - patch_sz + stride_sz) // stride_sz
            )**2
...
class VisualTransformerCls(nn.Module):
    def __init__(self, opt):
        super().__init__()
        self.encoder = VisualTransformer(
            opt.input_size,
            patch_sizes=opt.patch_sizes,
            patch_strides=opt.patch_strides,
            width=768,
            layers=12,
            heads=12,
            output_dim=512,
        ...
        self.classifier = Classfier(opt.num_classes)
