...
from vitlab.network.resnet.resnet import ResNet

from .base import ResNetBaseTrainer


__all__ = ['ResNet50Trainer', 'ResNet101Trainer']


class _ResNetTrainer(ResNetBaseTrainer):
    @gen.synchrony
    def __init__(self, opt: Opts) -> None:
        super().__init__(opt)
        net = ResNet(opt, self.backbone)
        self.optimizer = SGD(
...
@mod.register('arch')
class ResNet50Trainer(_ResNetTrainer):
    backbone = 'resnet50'


@mod.register('arch')
class ResNet101Trainer(_ResNetTrainer):
    backbone = 'resnet101'
